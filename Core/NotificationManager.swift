...
final class NotificationManager {
    ...
    private let lastTokenKey = "rydr.rider.notifications.lastFCMToken"
    private let lastUIDKey = "rydr.rider.notifications.lastUID"
    ...
    private func currentFCMToken() async throws -> String {
        try await withCheckedThrowingContinuation { continuation in
            Messaging.messaging().token { token, error in
                ...
                guard let token, !token.isEmpty else {
                    continuation.resume(throwing: NotificationManagerError.missingToken)
                    return
                }
    ...
    private static func log(_ event: String, _ fields: [String: String] = [:]) {
        let details = fields
            ...
            .map { "\($0.key)=\($0.value)" }
            .joined(separator: " ")
        print("[RydrNotifications][rider] \(event)\(details.isEmpty ? "" : " \(details)")")
    }
}
